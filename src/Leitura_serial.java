    ...
    public synchronized void serialEvent(SerialPortEvent oEvent) {
        if (oEvent.getEventType() == SerialPortEvent.DATA_AVAILABLE) {
            try {
                ...
                System.out.println("Recebido da serial: " + inputLine);

                // Supondo que a linha de entrada é no formato "temperatura,umidade"
                String[] data = inputLine.split(",");
                if (data.length == 2) {
                    try {
                        double temperatura = Double.parseDouble(data[0]);
                        double umidade = Double.parseDouble(data[1]);
                        Leitura leitura = new Leitura(temperatura, umidade);
                        leitura.salvarLeitura();
